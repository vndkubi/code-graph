...
async function listTools(extraArgs = []) {
...
test("fused MCP server exposes both CodeGraph and TokenOpt tools (client profile)", async () => {
  const names = await listTools();
  // CodeGraph surface
  assert.ok(names.includes("codegraph_context"), "expected codegraph_context");
  assert.ok(names.includes("codegraph_slice"), "expected codegraph_slice");
  assert.ok(names.includes("codegraph_status"), "expected codegraph_status");
  // TokenOpt/ContextGate surface
...
test("full profile widens both surfaces", async () => {
  const names = await listTools(["--mcp-profile", "full"]);
