...
import { PackProfile, normalizePackProfile, packProfileValue } from './pack-profile.js';
import {
  lineRangeString,
  compactFileCandidate,
  compactSymbolCandidate,
  compactSymbolList,
  uniqueFileCandidates,
  uniqueSymbolCandidates,
  compactEndpointCandidates,
  uniqueEndpointCandidates,
} from './packing.js';
import { findAffectedTests } from './affected-tests.js';
import {
...
import {
  slimTestsForPack,
  slimValidationForPack,
  slimTaskOracleForPack,
  taskOracleFor,
  normalizeOracleTaskType,
  taskKindForOracle,
} from './oracle.js';
import {
  normalizeRepoAtlasFormat,
  repoAtlasIncludesRole,
  atlasNumber,
  repoAtlasLanguageSummary,
  buildRepoAtlasFileModuleMap,
  buildRepoAtlasModules,
  buildRepoAtlasModuleDependencies,
  repoAtlasHotspot,
  repoAtlasFlowName,
  repoAtlasFlowTestSeeds,
  repoAtlasFlowConfidence,
  repoAtlasModuleRootForFile,
  repoAtlasModuleLabel,
  repoAtlasModuleId,
  renderRepoAtlasMarkdown,
  type RepoAtlasSnapshotRow,
  type RepoAtlasFileRow,
  type RepoAtlasHotspotRow,
  type RepoAtlasOverlayModuleRow,
  type RepoAtlasOverlayEdgeRow,
  type RepoAtlasModule,
} from './repo-atlas.js';
import {
  searchFiltersFor,
  fileFiltersFor,
  symbolFileLanguageFilter,
  referenceFiltersFor,
  hasTestIntent,
  hasGeneratedIntent,
  hasFixtureIntent,
  ...
  buildEndpointFacets,
  groupReferences,
  slimReferenceForBudget,
  slimDependencyTraceEdge,
  dependencyTraceGroups,
  copyDefined,
...
export class V2QueryService {
  ...
  // Session-scoped evidence ledger: snapshotId -> set of "file\0lines\0symbol"
  // keys whose SOURCE TEXT has already been handed to the agent this session.
  // The MCP runtime builds one V2QueryService per process (memoized in
  // proxy.ts), so this instance lives for the whole conversation. We use it to
  // avoid re-emitting identical source bodies on every pack call (the main
  // source of duplicate token spend across an investigate -> trace -> change
  // sequence). Keyed by snapshotId so a reindex (which can shift line ranges)
  // naturally invalidates stale entries.
  private readonly deliveredEvidence = new Map<string, { snapshotId: string; keys: Set<string> }>();
  ...
  private evidenceLedgerKey(file: unknown, lines: unknown, symbol: unknown): string | undefined {
    const f = typeof file === 'string' ? file : '';
    const l = typeof lines === 'string' ? lines : '';
  ...
  /**
   * Replace the source `text` of any evidence slice already delivered earlier
   * in this session with a compact reuse stub. The agent keeps the file/lines
   * handle so it can re-fetch with get_file_slice if the body scrolled out of
   * context, but in the common case it reuses what it was already given —
   * eliminating duplicate token spend. Pass `disable` (freshEvidence=true) to
   * force full bodies.
   */
  ...
  private async indexFreshness(
    ...
    const computedDirtyFiles = gitDirty || !git.available
      ? await this.computeDirtyFiles(snapshotId)
      : undefined;
    const dirtyCounts = (computedDirtyFiles ?? {}) as {
      addedCount?: number;
    ...
    // Only surface dirtyFiles when working-tree files actually changed. When the
    // staleness comes purely from a commit/branch switch on a clean tree, there
    // are no dirty files to list — returning an all-zero object misleads the
    // agent into thinking it has a file list to act on. Undefined = "stale, but
    // re-index rather than re-read specific files".
    const dirtyFiles = fileDirty ? computedDirtyFiles : undefined;
  ...
  private async searchFiles(snapshotId: string, args: Record<string, unknown>) {
    ...
    // mode here would return only the named class while ignoring the services,
    // controllers, and related files the question is actually asking about.
    const isResearchQuestion = /\b(how|does|what|why|trace|explain|understand|describe|walk|analyze|overview)\b/i.test(query);
    const useExplicitOnly = explicitRows.length > 0 && !broadConfigSearch && !isMyBatisIntent(query) && !isResearchQuestion;
    ...
    if (process.env.CG_DBG_SF) console.error('[sf]', JSON.stringify({ candidateTerms, rowsPool: rows.length, ranked: ranked.length, hasManifest: rows.some(r => r.path.includes('manifest')), hasIndexer: rows.some(r => r.path.includes('indexer')) }));
    const selected = ranked.slice(cursorOffset, cursorOffset + limit);
  ...
  private async symbolSearchCandidateRowsFromFts(
    ...
    // Order by BM25 so distinctive matches survive a small candidate window.
    // Without it FTS returns rows in docid order, so a high-frequency token
    // ("controller*", 36+ symbols) floods the window and the precise match
    // ("RecallsController", which also matches the rare "recall*") falls outside
    // the candidate set at small limits — search_symbol "RecallController" then
    // returned unrelated Admin*Controller classes. Over-fetch then keep the
    // top-ranked keys so scoring sees the genuine candidates.
    const overFetch = Math.min(candidateLimit * 4, 8000);
    const idRows = await this.db.prepare(`
      SELECT entity_id, bm25(codegraph_search_fts) AS rank
      ...
      ORDER BY rank
      LIMIT ?
    `).all(matchQuery, snapshotId, overFetch) as Array<{ entity_id: string }>;
  ...
  /**
   * Per-file BM25 relevance from the FTS index for the given query terms.
   * FTS5 bm25() returns lower (more negative) for better matches; we map it to a
   * small positive boost so a file whose indexed name/content is a strong
   * textual match ranks up, complementing (not overriding) the exact/compound
   * filename and symbol signals. Returns file -> boost in [0, cap].
   */
  /**
   * Call-graph in-degree (how many call edges target each symbol) for the given
   * fully-qualified names. A symbol called from many places is more central /
   * more likely to be what the user is looking for, so symbol ranking uses it
   * as a bounded tie-breaker — the symbol-side mirror of file dependency
   * centrality.
   */
  private async symbolCallInDegree(snapshotId: string, fqNames: string[]): Promise<Map<string, number>> {
    const out = new Map<string, number>();
    const unique = [...new Set(fqNames.filter(Boolean))];
    if (unique.length === 0) return out;
    for (const batch of chunksOf(unique, 500)) {
      const placeholders = batch.map(() => '?').join(', ');
      const rows = await this.db.prepare(`
        SELECT callee, COUNT(*) AS c
        FROM call_edges
        WHERE snapshot_id = ?
          AND signal_tier IN ('primary', 'provider')
          AND callee IN (${placeholders})
        GROUP BY callee
      `).all(snapshotId, ...batch) as Array<{ callee: string; c: number }>;
      for (const row of rows) out.set(row.callee, Number(row.c) || 0);
    }
    return out;
  }

  private async fileBm25Scores(
    snapshotId: string,
    terms: string[],
    files: string[],
    ...
    if (!matchQuery || unique.length === 0) return out;
    // bm25() is an FTS5 auxiliary function and cannot be wrapped in an
    // aggregate (MIN) — select per row and reduce to the best (lowest) rank per
    // file in JS.
    const bestRank = new Map<string, number>();
    for (const batch of chunksOf(unique, 500)) {
      const placeholders = batch.map(() => '?').join(', ');
      const rows = await this.db.prepare(`
        SELECT file, bm25(codegraph_search_fts) AS rank
        FROM codegraph_search_fts
        WHERE codegraph_search_fts MATCH ?
          AND snapshot_id = ?
          AND entity_type IN ('file', 'symbol')
          AND file IN (${placeholders})
      `).all(matchQuery, snapshotId, ...batch) as Array<{ file: string; rank: number }>;
      for (const row of rows) {
        const rank = Number(row.rank);
        if (!Number.isFinite(rank)) continue;
        const prev = bestRank.get(row.file);
        if (prev === undefined || rank < prev) bestRank.set(row.file, rank);
      }
    }
    for (const [file, rank] of bestRank) {
      // bm25 is <= 0 for matches; -rank grows with relevance. Compress with a
      // log curve and cap so it never dominates exact/compound boosts.
      const boost = Math.min(Math.round(Math.log1p(Math.max(0, -rank)) * 12), 25);
      if (boost > 0) out.set(file, boost);
    }
    return out;
  }
  ...
  private async fileSearchCandidateRowsFromFts(
    ...
    const matchQuery = ftsPrefixQuery(terms);
    if (!matchQuery) return [];
    // Order by BM25 relevance before deduplicating by file. Without this ordering,
    // a high-frequency OR term (e.g. "recall" matching 1000 files) fills the
    // candidate set with irrelevant results before the high-IDF matches
    // (e.g. "spaced*" → "SpacedRepetitionAlgorithm.java") are ever seen.
    // Over-fetch by 20× to ensure enough distinct files survive deduplication.
    const overFetch = Math.min(candidateLimit * 20, 10000);
    const rawRows = await this.db.prepare(`
      SELECT file, bm25(codegraph_search_fts) AS rank
      FROM codegraph_search_fts
      WHERE codegraph_search_fts MATCH ?
        AND snapshot_id = ?
      ...
      ORDER BY rank
      LIMIT ?
    `).all(matchQuery, snapshotId, overFetch) as Array<{ file: string; rank: number }>;
    const seenFiles = new Set<string>();
    const orderedFiles: string[] = [];
    for (const row of rawRows) {
      if (!seenFiles.has(row.file)) {
        seenFiles.add(row.file);
        orderedFiles.push(row.file);
      }
      if (orderedFiles.length >= candidateLimit) break;
    }
    if (orderedFiles.length === 0) return [];

    const rowsByFile = new Map<string, FileRow>();
    for (const batch of chunksOf(orderedFiles, 500)) {
      const placeholders = batch.map(() => '?').join(', ');
      const batchRows = await this.db.prepare(`
        SELECT f.path, f.language, f.file_role, f.parse_status, f.size
        FROM files f
        WHERE ${baseWhere}
          AND f.path IN (${placeholders})
      `).all(...baseParams, ...batch) as FileRow[];
      for (const row of batchRows) rowsByFile.set(row.path, row);
    }
  ...
  // Keyword endpoint fallback: a natural-language flow question ("how is a note
  // created through the REST API") names no explicit "/path", so endpointNeedleFromText
  // returns '' and impactedEndpoints stays empty even though the endpoint is indexed.
  // Match endpoints whose path OR handler symbol contains the query's domain tokens,
  // ranked by how many distinct tokens hit. Gated by the caller to endpoint/flow intent
  // so plain architecture questions do not pull in endpoint noise.
  private async findEndpointsByKeywords(
    snapshotId: string,
    tokens: string[],
    limit: number,
  ): Promise<Array<Record<string, unknown>>> {
    // Stem to a 5-char prefix so "creates" matches the "create-note" path and
    // "createNote" handler. Generic HTTP/subject words carry no routing signal.
    const needles = uniqueStrings(
      tokens
        .filter(token => token.length >= 4 && !ENDPOINT_GENERIC_TOKENS.has(token))
    ...
    const allRows = await this.db.prepare(`
      SELECT method, path, path_resolution, path_resolution_reason,
             handler_symbol, controller, file, line, framework, confidence, file_role
      FROM endpoints
      WHERE snapshot_id = ? AND file_role = 'main_source'
    `).all(snapshotId) as EndpointRow[];
    if (allRows.length === 0) return [];
    const haystacks = allRows.map(row => `${row.path ?? ''} ${row.handler_symbol ?? ''}`.toLowerCase());
    // Inverse endpoint-frequency weight: a token present in a large share of endpoints
    // ("user" — nearly every handler touches a user) is near-noise; a rare token
    // ("recall", "create") is the real selector. This is what stops the keyword
    // fallback from returning admin/user endpoints for a "create note" question.
    const docFreq = new Map<string, number>();
    for (const needle of needles) {
      docFreq.set(needle, haystacks.filter(haystack => haystack.includes(needle)).length || 1);
    }
    const total = allRows.length;
    const scored = allRows.map((row, index) => {
      const haystack = haystacks[index];
      let score = 0;
      let hits = 0;
      for (const needle of needles) {
        if (haystack.includes(needle)) {
          // Smoothed IDF: a match always contributes (base 1) so a single-endpoint
          // corpus still scores, while a rare token adds discrimination on top — a
          // token present in every endpoint (idf 0) must not zero the whole match out.
          score += 1 + Math.log(total / (docFreq.get(needle) ?? 1));
          hits += 1;
        }
      }
      return { row, score, hits };
    }).filter(entry => entry.hits > 0 && entry.score > 0);
    scored.sort((a, b) => b.score - a.score || b.row.confidence - a.row.confidence || a.row.path.localeCompare(b.row.path));
    return scored.slice(0, limit).map(entry => endpointDto(entry.row));
  }
  ...
  private async reviewPatch(snapshotId: string, args: Record<string, unknown>) {
    ...
    // Graph-reachable tests: the set a reviewer must actually run for this change,
    // via the reverse dependency graph (accurate for Java, not just name proximity).
    const reachableTests = changedFiles.length > 0
      ? (await findAffectedTests(this.db, snapshotId, changedFiles, { maxDepth: 5, limit })).tests
      : [];
    const reachableSet = new Set(reachableTests.map(t => t.file));
    const nameTests = arrayRecords(impact.testsLikelyRelevant);
    const tests: Array<Record<string, unknown>> = [
      ...reachableTests.map(t => ({
        file: t.file,
        distance: t.distance,
        via: t.via,
        reason: `reaches the change in ${t.distance} dependency hop(s) via ${t.via.split('/').pop()}`,
      })),
      ...nameTests.filter(t => { const f = String((t as { file?: unknown }).file ?? ''); return f !== '' && !reachableSet.has(f); }),
    ];
    // "Untested change" risk: changed implementation files with NO test reachable
    // through the dependency graph — exactly what a reviewer must be warned about.
    const changedMainSource = changedFiles.length > 0
      ? (await this.db.prepare(`
          SELECT path FROM files
          WHERE snapshot_id = ? AND file_role = 'main_source'
            AND path IN (${changedFiles.map(() => '?').join(', ')})
        `).all(snapshotId, ...changedFiles) as Array<{ path: string }>).map(r => r.path)
      : [];
    // impact.riskFlags was computed inside simulatePatchImpact() using only
    // name-proximity test matching, before the graph-reachable tests above were
    // merged in. Drop a stale 'no-tests-found' flag once reachable/name tests
    // are known, so the packet doesn't emit review-missing-tests while also
    // listing relevant tests in testsLikelyRelevant.
    const riskFlags = arrayRecords(impact.riskFlags)
      .filter(flag => !(String(flag.type ?? '') === 'no-tests-found' && tests.length > 0));
    if (changedMainSource.length > 0 && reachableTests.length === 0) {
      riskFlags.unshift({
        type: 'untested-change',
        severity: 'high',
        reason: 'No test reaches the changed implementation through the dependency graph; the change may be unverified by the existing suite.',
        evidence: changedMainSource.slice(0, 5),
  ...
  private async patchReviewTargets(
    ...
    for (let index = 0; index < targetHunks.length; index++) {
      ...
      const resolvedFile = await this.resolveFile(snapshotId, hunk.file) ?? hunk.file;
      // The import/call graph of a test, mock, or generated file carries little
      // review value (its callees are framework/test calls, its dependencies are
      // test imports) yet dominated the review packet — graphContext was ~50% of
      // the bytes and most targets were test-file hunks. For those roles keep the
      // counts as a signal but drop the verbose evidence rows.
      const targetFileRole = resolvedFile
        ? ((await this.db.prepare('SELECT file_role FROM files WHERE snapshot_id = ? AND path = ?')
            .get(snapshotId, resolvedFile)) as { file_role?: string } | undefined)?.file_role
        : undefined;
      const lowImpactGraphRole = targetFileRole === 'test_source'
        || targetFileRole === 'mock_source'
        || targetFileRole === 'generated';
      ...
      targets.push({
        ...
        graphContext: {
          callers: lowImpactGraphRole ? [] : callers.slice(0, maxEvidence).map(compactCallEdge),
          callees: lowImpactGraphRole ? [] : callees.slice(0, maxEvidence).map(compactCallEdge),
          dependents: lowImpactGraphRole ? [] : compactReviewObject(dependents, maxEvidence, 2),
          dependencies: lowImpactGraphRole ? [] : compactReviewObject(dependencies, maxEvidence, 2),
          endpoints: lowImpactGraphRole ? [] : compactReviewObject(endpoints, maxEvidence, 2),
          ...(lowImpactGraphRole ? { evidenceOmittedForRole: targetFileRole } : {}),
          counts: {
  ...
  private async findTestsFor(snapshotId: string, args: Record<string, unknown>) {
    ...
    const nameTests = await this.findRelevantTests(snapshotId, target, limit);

    // Graph-reachable tests: resolve the change to file(s) and walk the reverse
    // dependency graph. Precise where name proximity fails — a test that exercises
    // calculateNextRecallAt only through RecallService never names it, so the
    // name/call-edge heuristic alone returned nothing for such changes.
    const seedFiles = stringArray(args.files);
    if (args.file) seedFiles.push(String(args.file));
    if (seedFiles.length === 0 && target) {
      seedFiles.push(...await this.filesDeclaringSymbol(snapshotId, target));
    }
    const reachable = seedFiles.length > 0
      ? (await findAffectedTests(this.db, snapshotId, seedFiles, { maxDepth: 5, limit })).tests
      : [];

    // Merge: graph-reachable first (a real dependency path is stronger evidence
    // than a name match), then name-proximity tests not already present.
    const seen = new Set<string>();
    const tests: Array<Record<string, unknown>> = [];
    for (const t of reachable) {
      if (seen.has(t.file)) continue;
      seen.add(t.file);
      tests.push({ file: t.file, distance: t.distance, via: t.via, reasons: [`reaches change in ${t.distance} dependency hop(s) via ${t.via.split('/').pop()}`] });
    }
    ...
    const trimmed = tests.slice(0, limit);
    return {
      target,
      tests: trimmed,
      totalCount: trimmed.length,
      reachableCount: reachable.length,
      confidence: trimmed.length > 0 ? (reachable.length > 0 ? 0.8 : 0.6) : 0.35,
      confidenceNotes: [
        'Tests reachable through the reverse dependency graph (import + resolved file edges, works for Java) are listed first; remaining tests are ranked by file/name proximity and call edges.',
      ],
    };
  }

  /** Main-source files that declare a class/interface/method named `target`. */
  private async filesDeclaringSymbol(snapshotId: string, target: string): Promise<string[]> {
    const name = target.includes('.') ? target.substring(target.lastIndexOf('.') + 1) : target;
    if (!name) return [];
    const rows = await this.db.prepare(`
      ...
      WHERE snapshot_id = ?
        ...
        AND (simple_name = ? OR fq_name = ? OR fq_name LIKE ? ESCAPE '\\')
      LIMIT 10
    `).all(snapshotId, name, target, `%.${escapeLike(name)}`) as Array<{ file: string }>;
    return uniqueStrings(rows.map(r => r.file));
  }
  ...
  private async getResearchPack(snapshotId: string, args: Record<string, unknown>) {
    ...
    const rightSize = args.rightSize !== false;
    // Default 0.5 (not 0.8): broad multi-file tasks need the whole relevant
  // cluster, and the cost of one extra file (~200 tok) is far below the cost of
  // a dropped one (a re-query loop, ~3k tok). Calibrated on the broad-task
  // benchmark where 0.8 regressed recall while 0.5 matched the no-trim baseline.
  const keepRatio = typeof args.keepRatio === 'number' ? Math.max(0.1, Math.min(1.0, Number(args.keepRatio))) : 0.5;
    ...
    // Keyword endpoint fallback (bug: impactedEndpoints empty for NL flow questions with
    // no explicit "/path"). Populate the informational field only — NOT a handler seed —
    // so the pack's resolution path is unchanged. Gated to endpoint/flow intent.
    const endpointIntent = /\b(api|endpoint|route|rest|http|handler|request flow|controller)\b/i.test(target);
    const keywordEndpointCandidates = (endpointNeedle === '' && endpointIntent && endpointCandidates.length === 0)
      ? compactEndpointCandidates(await this.findEndpointsByKeywords(snapshotId, tokenizeSearchQuery(target), packProfileValue(profile, 3, 4, 5)))
      : [];
    const impactedEndpoints = uniqueEndpointCandidates([
      ...exactEndpointCandidates,
      ...endpointCandidates,
      ...keywordEndpointCandidates,
    ...
    const symbolCandidates = uniqueSymbolCandidates([
      ...
      ...explicitSymbols,
      ...(rightSize ? rightSizeCandidates(symbolRows, { keepRatio }) : symbolRows).map(row => compactSymbolCandidate(row)),
    ]);
    ...
    const lexicalCandidatePool = uniqueFileCandidates([
    ...
    const structuralExpansion = !hasEndpointSeed && !explicitFastPath;
    // Centrality: domain hubs whose name covers >=2 query words and are depended
    // on by many files. Lexical ranking buries these when their distinctive token
    // is high-frequency (AuthorizationService: "authorization"+"service",
    // in-degree 31, but intent boosts pushed it past the cap). Keep structural
    // role words (service/controller/...) as match words even though the general
    // tokenizer treats them as stop words, since they make a name multi-word.
    const ROLE_MATCH_WORDS = new Set(['service', 'controller', 'repository', 'entity', 'dao', 'dto', 'mapper', 'handler', 'resolver', 'manager', 'factory', 'job', 'validator', 'config', 'provider', 'listener', 'interceptor', 'filter']);
    const centralTokens = uniqueStrings(splitIdentifierWords(target))
      .filter(token => token.length >= 3 && (ROLE_MATCH_WORDS.has(token) || !RESEARCH_STOP_WORDS.has(token)));
    const centralFilePaths = structuralExpansion
      ? await this.researchCentralFiles(snapshotId, centralTokens, preferredLanguage, packProfileValue(profile, 2, 3, 3))
      : [];
    const centralCandidates = centralFilePaths.length > 0
      ? (await this.fileCandidatesForPaths(snapshotId, centralFilePaths, target)).map(candidate => ({
        ...candidate,
        whyRelevant: `structural: domain hub (name covers multiple query words, high dependency in-degree); ${candidate.whyRelevant}`,
      }))
      : [];
    // Graph expansion: pull 1-hop import/dependency neighbors of the strongest
    // lexically-found seeds AND the centrality hubs. These are structurally
    // central files a lexical search misses (the controller that calls the seed
    // service, etc.). Skip when an endpoint or explicit file already anchors the
    // pack — those paths run their own callee-definition expansion below.
    const graphSeedFiles = uniqueStrings([
      ...lexicalCandidatePool.slice(0, 5).map(file => file.file),
      ...centralFilePaths,
      ...relevantSymbols.map(symbol => symbol.file),
    ]).filter(Boolean);
    const graphNeighborFiles = (structuralExpansion && graphSeedFiles.length > 0)
      ? await this.researchGraphNeighborFiles(snapshotId, graphSeedFiles, preferredLanguage, packProfileValue(profile, 3, 4, 5))
      : [];
    const graphExpandedCandidates = graphNeighborFiles.length > 0
      ? (await this.fileCandidatesForPaths(snapshotId, graphNeighborFiles, target)).map(candidate => ({
        ...candidate,
        whyRelevant: `structural: 1-hop import/dependency neighbor of a ranked seed (graph expansion); ${candidate.whyRelevant}`,
      }))
      : [];
    const candidateCap = packProfileValue(profile, 4, 5, 6);
    // Reserve cap slots for structural signal (centrality + graph) so it is never
    // squeezed out by the lexical cluster; the remainder stays lexical, and any
    // overflow lexical files dedupe in after. Many structural hits overlap the
    // lexical set (a strong lexical file is often also a hub) so the effective
    // rotation is smaller than the nominal quota.
    const centralQuota = Math.min(centralCandidates.length, packProfileValue(profile, 1, 1, 2));
    const graphQuota = Math.min(graphExpandedCandidates.length, packProfileValue(profile, 1, 2, 2));
    const reservedQuota = Math.min(candidateCap - 1, centralQuota + graphQuota);
    const candidateFiles = uniqueFileCandidates([
      ...lexicalCandidatePool.slice(0, candidateCap - reservedQuota),
      ...centralCandidates.slice(0, centralQuota),
      ...graphExpandedCandidates.slice(0, graphQuota),
      ...lexicalCandidatePool.slice(candidateCap - reservedQuota),
    ...
    // Resolve the defining files of the top direct callees — the "first downstream
    // service" hop — for EVERY research flow, not just endpoint-seeded ones. Prior
    // to this, architecture/"how does X work" questions (which rarely seed on an
    // endpoint) listed downstream services as callee edges but never pulled their
    // source into evidence: measured downstream-service coverage was 27%. The JDK/
    // stdlib callee filter (isLibraryCallOwner) already strips `List`/`Map`/etc from
    // calleesRaw, so resolving definitions now yields real domain services
    // (NoteMotionService, MemoryTracker, EntityPersister...) rather than stdlib noise,
    // making this expansion safe to run unconditionally. The evidenceSlices budget
    // still bounds how much downstream source is actually inlined.
    const calleeDefinitions = calleesRaw.length > 0
    ...
    const calleeFileCandidates: ResearchFileCandidate[] = calleeDefinitions.map(symbol => ({
      file: symbol.file,
      lines: symbol.lines,
      whyRelevant: `downstream callee definition: ${symbol.name}`,
    ...
    const topFiles = uniqueFilesInOrder([
      ...
      //    callee files above it regressed recall 0.64->0.51).
      ...candidateFiles.map(file => file.file),
      // 2. Pure-FTS file matches for the LEFTOVER slots — ahead of the symbol-derived
      //    lexical pool and callee/flow files. These are ranked purely by how well the
      //    file/path/symbol text matches the query, so a phrase match (RecallPrompt for
      //    "recall prompts", NoteRecallSetting for "note recall settings") beats a
      //    generic symbol-search hit (e.g. User* pulled in by the bare seed "user").
      //    Untrimmed on purpose: rightSizeCandidates trims the pool to bound EVIDENCE-
      //    snippet tokens, but topFiles is a cheap path list. Placed AFTER candidateFiles
      //    so it only fills slots the primary core left open — never displaces a primary.
      ...fileResults.files.map(row => String((row as { path?: unknown }).path ?? '')),
      // 3. Symbol-derived lexical pool next.
      ...lexicalCandidatePool.map(file => file.file),
      // 4. Structural downstream (callee defs, flow symbols, call edges) fill whatever
      //    remains.
    ...
    ]).slice(0, packProfileValue(profile, 6, 8, 10));
    ...
    // that landed in a fill slot (e.g. RecallPrompt at position 7-8) was being clipped
    // off by the tighter return cap even though it was ranked. Monotonic for recall.
    const returnedTopFiles = topFiles.slice(0, packProfileValue(profile, 6, 8, 10));
    ...
    const evidenceDedup = this.dedupePackEvidenceSlices(
      snapshotId,
    ...
    const packedEvidenceSlices = evidenceDedup.slices;
  ...
  private async researchCallEdges(
    ...
    // Drop JDK/stdlib callee targets (`Arrays.stream`, `Integer.valueOf`, ...):
    // they pass the `signal_tier IN ('primary','provider')` gate but carry no
    // domain-flow meaning, yet dominate a method's callee list (~22% of the
    // investigate packet). Only filter the 'callees' direction — in 'callers'
    // the domain symbol is the callee (the needle) and must be kept.
    const domainRows = direction === 'callees'
      ? rows.filter(row => !isLibraryCallOwner(row.callee))
      : rows;
    return uniqueCallEdges(domainRows)
      .sort((a, b) => researchCallEdgeScore(b) - researchCallEdgeScore(a) || a.line - b.line || a.callee.localeCompare(b.callee))
      .slice(0, limit);
  }
  ...
  private async researchCalleeDefinitions(
    ...
    for (const edge of edges) {
      ...
      const symbol = compactSymbolCandidate(
        symbolDto(row),
        `definition for downstream callee ${edge.callee}`,
      );
  ...
  const keepRatio = typeof args.keepRatio === 'number' ? Math.max(0.1, Math.min(1.0, Number(args.keepRatio))) : 0.5;
    ...
    const fuzzyFileCandidates = (rightSize ? rightSizeCandidates(files.files, { keepRatio, minKeep: 3 }) : files.files)
    ...
    const coversSpecificToken = (filePath: string): boolean => {
      // Stem-aware: a path word `index` covers the query token `indexing`.
      // Exact-match here wrongly declared the main-source answer uncovered,
      // which fired the test/fixture fallback and let a literally-named test
      // file (`full-pipeline.test.ts` covering "pipeline") surface as the top
      // candidate ahead of the real implementation.
      const words = splitIdentifierWords(filePath);
      return specificTaskTokens.some(token => words.some(word => wordsShareStem(token, word)));
    };
    ...
    const relevantSymbols = uniqueSymbolCandidates([
      ...
      ...(rightSize ? rightSizeCandidates(symbols.symbols, { keepRatio }) : symbols.symbols).map(row => compactSymbolCandidate(row)),
    ]).slice(0, maxSymbols);
  ...
  /**
   * Graph expansion — the structural-recall lever that lexical search cannot
   * match. Given seed files already found lexically, returns their 1-hop
   * import/dependency neighbors (both directions) ranked by how many seeds touch
   * each one. This recovers files that are structurally central to the task but
   * never repeat its terms — e.g. for "trace the recall flow", the controller
   * that calls RecallService but is named "RecallsController" and shares no
   * distinctive tokens with the query. Restricted to main_source and the
   * preferred language so test/generated/frontend neighbors don't dilute the set.
   */
  private async researchGraphNeighborFiles(
    snapshotId: string,
    seedFiles: string[],
    preferredLanguage: string | undefined,
    limit: number,
  ): Promise<string[]> {
    const seeds = [...new Set(seedFiles.filter(Boolean))].slice(0, 12);
    ...
    const edges = await this.db.prepare(`
      ...
      WHERE snapshot_id = ?
        AND (from_file IN (${placeholders}) OR to_file IN (${placeholders}))
    `).all(snapshotId, ...seeds, ...seeds) as Array<{ from_file: string; to_file: string }>;
    const seedSet = new Set(seeds);
    // Each edge contributes its non-seed endpoint; rankFiles then orders by how
    // many seeds connect to it, so a hub used across the seed cluster ranks first.
    const neighborHits = edges.flatMap(edge => {
      const out: string[] = [];
      if (seedSet.has(edge.from_file) && edge.to_file && !seedSet.has(edge.to_file)) out.push(edge.to_file);
      if (seedSet.has(edge.to_file) && edge.from_file && !seedSet.has(edge.from_file)) out.push(edge.from_file);
      return out;
    });
    const ranked = rankFiles(neighborHits);
    if (ranked.length === 0) return [];
    const top = ranked.slice(0, Math.max(limit * 4, 24));
    const rolePlaceholders = top.map(() => '?').join(', ');
    const rows = await this.db.prepare(`
      SELECT path, file_role, COALESCE(language, '') AS language
      FROM files
      WHERE snapshot_id = ? AND path IN (${rolePlaceholders})
    `).all(snapshotId, ...top) as Array<{ path: string; file_role: string; language: string }>;
    const meta = new Map(rows.map(row => [row.path, row]));
    return top.filter(file => {
      const m = meta.get(file);
      if (!m || m.file_role !== 'main_source') return false;
      if (preferredLanguage && m.language && m.language !== preferredLanguage) return false;
      return true;
    }).slice(0, limit);
  }

  /**
   * Centrality-augmented candidates — the second structural lever. A file whose
   * camel-split name covers multiple query words AND is depended on by many other
   * files (high dependency in-degree) is a genuine domain hub, even when lexical
   * ranking buries it (e.g. AuthorizationService: covers "authorization"+"service",
   * in-degree 31, but generic controller/intent boosts pushed it past the cap).
   * Returns main_source files in the preferred language whose basename covers >= 2
   * distinct query tokens, ranked by coverage then in-degree. In-degree is only a
   * tiebreaker within equal coverage so a globally-central but weakly-matched file
   * (User.java) never wins on centrality alone.
   */
  private async researchCentralFiles(
    snapshotId: string,
    tokens: string[],
    preferredLanguage: string | undefined,
    limit: number,
  ): Promise<string[]> {
    const queryWords = new Set(tokens.filter(token => token.length >= 3));
    if (queryWords.size < 2) return [];
    const langClause = preferredLanguage ? 'AND language = ?' : '';
    const fileRows = await this.db.prepare(`
      SELECT path FROM files
      WHERE snapshot_id = ? AND file_role = 'main_source' ${langClause}
    `).all(...[snapshotId, ...(preferredLanguage ? [preferredLanguage] : [])]) as Array<{ path: string }>;
    if (fileRows.length === 0) return [];
    const indegRows = await this.db.prepare(`
      SELECT to_file AS file, COUNT(DISTINCT from_file) AS indeg
      FROM dependency_edges
      WHERE snapshot_id = ?
      GROUP BY to_file
    `).all(snapshotId) as Array<{ file: string; indeg: number }>;
    const indeg = new Map(indegRows.map(row => [row.file, row.indeg]));
    const scored = fileRows.map(row => {
      const basename = (row.path.replace(/\\/g, '/').split('/').pop() ?? row.path).replace(/\.[^./]+$/, '');
      const words = new Set(splitIdentifierWords(basename));
      let coverage = 0;
      for (const word of words) if (queryWords.has(word)) coverage += 1;
      return { file: row.path, coverage, indeg: indeg.get(row.path) ?? 0 };
    }).filter(item => item.coverage >= 2);
    scored.sort((a, b) => b.coverage - a.coverage || b.indeg - a.indeg || a.file.localeCompare(b.file));
...
// Generic endpoint/HTTP vocabulary that carries no routing signal — excluded from the
// keyword endpoint fallback so it matches on the DOMAIN tokens (e.g. "note"/"create"),
// not on every endpoint that merely lives behind a controller.
const ENDPOINT_GENERIC_TOKENS = new Set([
  'api', 'apis', 'endpoint', 'endpoints', 'route', 'routes', 'rest', 'http', 'https',
  'handler', 'controller', 'request', 'response', 'flow', 'trace', 'through', 'database',
  'get', 'post', 'put', 'delete', 'patch', 'call', 'calls', 'method', 'url', 'path',
]);
...
function researchSeedTerms(target: string): string[] {
  ...
  // Capitalized words at sentence start ("Trace from...", "Implement note...")
  // are NOT class names. Without this filter they leak in as symbol seeds and the
  // pack resolves garbage definitions (e.g. "Trace" -> resolveTraceRoot in a CI
  // script), polluting flowSteps/topFiles and making the packet untrustworthy so
  // the agent falls back to shell anyway. Mirrors the search_files fix.
  const symbolTerms = (target.match(/\b(?:[A-Z][A-Za-z0-9_$]{2,}|[a-z]+[A-Z][A-Za-z0-9_$]*)\b/g) ?? [])
    .filter(term => !EXPLICIT_FILENAME_STOP_WORDS.has(term)
      && !RESEARCH_STOP_WORDS.has(term.toLowerCase())
      && !FILE_SEARCH_STOP_WORDS.has(term.toLowerCase()));
  const tokenTerms = tokenizeSearchQuery(target)
    .filter(token => token.length >= 3 && !RESEARCH_STOP_WORDS.has(token) && !FILE_SEARCH_STOP_WORDS.has(token))
    .slice(0, 12);
...
function calleeDefinitionScore(row: SymbolRow, edge: CallEdgeRow): number {
  ...
  // A constructor callee `Owner.new` should resolve to the `Owner` class/record.
  // Strip the `.new` suffix so the owner name still matches the type symbol, and
  // bias toward the type kind (the whole point is to land on the class, not a
  // like-named method).
  if (callee.endsWith('.new')) {
    const owner = callee.slice(0, -'.new'.length);
    if (owner === simple || owner.endsWith(`.${simple}`)) {
      score += 30;
      if (row.kind === 'class' || row.kind === 'record' || row.kind === 'interface' || row.kind === 'enum') score += 20;
    }
  }
...
function compressedEvidenceForResearch(input: {
  ...
  // sourceSliceRefs are pointers into the always-present top-level evidenceSlices,
  // NOT a second copy of the source. The prior `textPreview` field re-emitted the
  // first 180 chars of slice.text — text the packet already ships in full under
  // evidenceSlices — so it cost tokens (~a third of this block) for zero new
  // information. compressedEvidence never travels without evidenceSlices (answer
  // mode omits this whole block; agent mode ships both), so the pointer alone is
  // enough to say "open evidenceSlice at file:lines for exact text".
  ...
  const compressedTokens = estimateTokens(JSON.stringify({ factCards, sourceSliceRefs }));
  return {
    // The call graph is deliberately NOT restated here: the top-level callers/callees
    // arrays carry the same edges (filtered + richer), so a `callGraphEdges` copy was
    // ~8% of the packet in pure duplication. Read those for the call graph.
    intendedUse: 'Read this first for small-model routing; open full evidenceSlices only when a fact needs exact source text, and callers/callees for the call graph.',
    factCards,
    sourceSliceRefs,
...
function ftsPrefixQuery(terms: string[]): string | undefined {
  const tokens = uniqueStrings(terms
    ...
    // Require length >= 3 so file-extension tokens like "ts", "py", "js" (from
    // "Trace.ts", "Trace.py" in explicit needles) never become "ts*"/"py*" prefix
    // queries that match every TypeScript/Python file in the workspace.
    .filter(token => /^[a-z0-9]+$/.test(token) && token.length >= 3))
    .slice(0, 12);
...
function callEdgeLookupTerms(symbol: string): string[] {
  ...
  // Constructor callees are recorded as `Owner.new` (e.g. `new AiNoteAutomationService(...)`
  // -> `AiNoteAutomationService.new`). None of the derived terms below is the bare owner,
  // so the LIKE/exact lookup never finds the class symbol and the downstream service it
  // constructs is silently dropped from calleeDefinitions. Emit the owner so the class
  // resolves via `simple_name = ?`.
  const constructorOwner = /\.new$/.test(withoutParams)
    ? withoutParams.slice(0, -'.new'.length).split('.').filter(Boolean).pop() ?? ''
    : '';
  return uniqueStrings([
