...
watcher batches changed paths and uses path-delta refresh for small local
edits, falling back to a normal refresh only when the path set is missing or
too large. Tune the cutoff with `CODEGRAPH_PATH_DELTA_FILE_LIMIT`.
...
runbook, see [`docs/copilot-two-local-clones.md`](copilot-two-local-clones.md).
...
| MCP exits with `Timed out waiting for codegraph daemon to start` | Stale daemon metadata from a prior Docker MCP container, or Postgres lock contention from an aborted index | Use the current build, which removes stale daemon metadata on startup. If it still fails, inspect `/codegraph-home/logs/daemon.jsonl.bootstrap.log` in the cache volume and clear the stuck index/Postgres process. |
