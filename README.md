...
| `better-sqlite3`/`node-gyp` tries to download headers from `unofficial-builds.nodejs.org` | Alpine native addon build is trying to fetch Node headers over corporate TLS | Use the latest Dockerfile, which sets `NPM_CONFIG_NODEDIR=/usr/local` and builds native addons from source against local headers |
